...
<body>
	<a href="#list-recipe" class="skip" tabindex="-1"><g:message
	...
	<div id="list-recipe" class="content scaffold-list" role="main">
		...
		<div class="indent">
			<g:each in="${categoryInstanceList}" var="categoryInstance">
				...
				<div id="cat_${categoryInstance.id}" style="display: none"
					class="indent">
					<h3>
					...
					<div id="nosub_${categoryInstance.id}" style="display: none">
						<table>
							<g:each in="${categoryInstance.recipes}" status="ri"
								var="recipeInstance">
					...
					<g:each in="${categoryInstance.subCategories}"
						var="subCategoryInstance">
						<h3>
							<a href="#"
								onclick="switchMenu('sub_${subCategoryInstance.id}');"> ${fieldValue(bean: subCategoryInstance, field: "name")}
							</a>
						</h3>
						<div id="sub_${subCategoryInstance.id}" style="display: none">
							<table>
								<g:each in="${subCategoryInstance.recipes}" status="ri"
									var="recipeInstance">
									<tr class="${(ri % 2) == 0 ? 'even' : 'odd'}">
										<td><g:link action="show" id="${recipeInstance.id}">
												${fieldValue(bean: recipeInstance, field: "title")}
